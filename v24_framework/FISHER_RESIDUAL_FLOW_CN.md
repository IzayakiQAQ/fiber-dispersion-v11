...
可部署模型必须使用与评价运行相互独立的采集数据标定。把同一次运行按时间切成前 500 组标定、后 500 组留出，只能检查同一运行内的泛化，不能称为外部零参考验证。这里的独立要求不仅覆盖 PSF/模板，还覆盖 RL 迭代数、中心窗口、平滑尺度、门控阈值和候选选择规则。代码要求显式传入 `calibration_is_independent=True`；若为 `False` 会直接拒绝构造残差模型。
...
| 方法 | 280 Hz 数据是否参与模型构造/选择 | FWHM 中位数 | 10 s TDEV | 结论 |
|---|---|---:|---:|---|
| 物理生成 PSF + 事后迭代数选择 | PSF 构造否；`R` 选择使用前 500 组 | 约 155.9 ps | 全量 2.827 ps；后 500 组 2.841 ps | 仅 PSF 独立诊断，不是完全盲测 |
| Poisson/Fisher 残差诊断 | 前 500 组构造模板 | 约 155.9 ps | 全量 2.365 ps；后 500 组 2.490 ps | 仅同运行诊断，不是外部结果 |
| 全参数独立冻结后的一次性盲测 | 尚未执行 | -- | -- | 当前没有可声明结果 |

此前 `2.365/2.490 ps` 使用了 280 Hz 前 500 组构造 Poisson 模板，因此从外部结果声明中撤回。`2.827/2.841 ps` 虽然使用物理生成 PSF，但迭代数同样参考了前 500 组，因此也不作为完全零参考结果。该批数据的交叉功率 Fisher 审计属于事后分析，不是模型先验。当前必须先在独立 100 Hz 数据或预先声明的物理仿真上冻结全部参数，然后才可对 280 Hz 做一次性盲测。
