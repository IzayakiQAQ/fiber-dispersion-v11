...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-root",
        type=Path,
        default=Path("E:/lzy") / "测试结果" / "2026.6.29 50km 280Hz",
    )
    parser.add_argument(
        "--target-psf",
        type=Path,
        default=THIS_DIR / "models" / "physical_0km_target_psf.npz",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=THIS_DIR / "results" / "external_1000",
    )
...
def main() -> None:
    ...
    summary = {
        "framework": "v24_direct_histogram_compensator",
